...
@log
def encode_to_png(input_file: str, output_file: str, delete: bool) -> tuple:
    """
    编码输入图片到webp格式
...
usage: webp2png.py [options] -i <directory>
...
def print_error():
    ...
    :return:
    """
    print('''usage: webp2png.py [options] -i <directory>
...
def main(argv: list):
    ...
    if not os.path.isdir(flag.get('input')):
        ...
        exit(2)
    pool = Pool()
    for f in get_pics(flag['input'],('.webp')):
        o, ext = os.path.splitext(f)
        o += '.png'
        pool.apply_async(encode_to_png, (f, o, flag.get('delete', False)))
    pool.close()
    pool.join()
